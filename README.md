<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 103 项目横幅" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 103｜鲜彩抽象拼

### 把照片提炼成鲜活色块、粗粝拼合与高级留白
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 103 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

照片里真正让人记住的，往往不是所有细节，而是一个轮廓、一段关系或一种情绪。**Panel 103** 会保留上方真实照片，并在设计区域把这些关键线索压缩为鲜活的大色块、手绘线条与有呼吸感的抽象拼合，让复杂画面变得轻快、清晰，又保有原场景的精神核心。

### 适合这些情况

- **原图信息太多，视觉重点不够明确：** 把主体、姿态和叙事关系压缩为少量有力量的块面，让主题更快被看见。
- **想做抽象设计，又怕认不出原来的内容：** 保留最有识别度的轮廓和视觉隐喻，不把抽象变成无关装饰。
- **希望颜色鲜活治愈，但不想出现廉价糖果感：** 从原图挑选最有生命力的颜色重新调制，以暖白留白控制明度与节奏。
- **需要一套可以继续延展的视觉语言：** 适合艺术海报、独立出版、展览视觉、社交封面、壁纸和目录批量处理。

### 快速开始

发送一张图片给 Agent，并说明：

> 用 XXD Panel 103 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 抽象大色块 · 粗粝拼合 · 手工错位 · 原图鲜彩 · 编辑留白 · 适配字体

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 103 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张以鲜活大色块、粗粝拼合、专业布局和艺术留白共同成立的抽象编辑海报 |
| **一眼可辨的特点** | 不规则块面、剪贴边缘、手绘线、错位叠压、暖白呼吸空间与少量适配文字 |
| **它如何尊重你的输入** | 保留主体身份、姿态、关系和气氛，从照片中提炼颜色，不擅自替换核心叙事 |
| **可以用在哪里** | 独立出版、展览海报、艺术封面、社交内容、纯设计图、多比例成品和四端壁纸 |
<!-- xxd-panel-benefit:end -->

## 它如何把你的输入变成独特成品

```text
辨认最值得记住的主体与关系 → 删除完整场景和冗余细节 → 压缩为少量大色块与轮廓 → 通过错位、叠压、裁切和留白重组 → 提亮原图中最有生命力的颜色 → 用少量编辑文字完成视觉节奏
```

## 成品中最容易识别的风格特点

- 大色块不追求写实描摹，却通过主体轮廓、姿态和关系保留原图的精神核心。
- 不规则剪贴边缘、粗细不同的手绘线、局部覆盖与缝隙带来受控的即兴感。
- 天空蓝、湖水青、嫩绿、暖黄、珊瑚橙或桃粉只从原图自然推导，并以暖白底色留出呼吸。
- 视觉重心、正负形、疏密节奏和留白让画面看似随性，实际保持专业平衡。
- 少量文字介于手写标题与现代排版之间；拒绝写实插画、复杂背景、卡通感、模板化和强电脑生成感。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-103-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 103 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-103 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-103 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-103 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 103；公共设置只解析一次
/xxd-panel-103 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-103`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-103-prompt.en.md) · [中文运行适配器](references/xxd-panel-103-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

103 个 Panel 各自保留独立的原始提示词与审美逻辑。下面列出全部项目地址和核心风格特点；当前项目以粗体标出。
...
| **[xxd-panel-103](https://github.com/nevertoday/xxd-panel-103)** | 鲜彩抽象拼 · 不规则大色块 · 粗糙即兴 · 手绘线条 · 专业布局 · 艺术留白 · 适配字体 |
...
### 小小东总控 · 将军总指挥 Skill · 103 元

一次支付 103 元获得本套将军总指挥 Skill（`xxd-panel-all`），用于主控、推荐、点将和批量调度全部士兵 Skills。扫码后请备注“将军总指挥 Skill”。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–103） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
