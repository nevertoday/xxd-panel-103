<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 103 프로젝트 배너" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 103｜선명한 추상 조합

### 사진을 생생한 색면, 거친 조합, 예술적 여백으로 압축합니다
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 103이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

사진에는 포스터에 필요한 것보다 많은 세부가 담기곤 합니다. **Panel 103**은 현실 사진을 유지하면서 기억에 남는 윤곽, 관계, 감정을 생생한 색면, 손그림 선, 숨 쉴 공간이 있는 추상 조합으로 압축합니다. 원래 장면의 정신적 핵심은 잃지 않으면서 더 가볍고 명확한 화면을 만듭니다.

### 이런 경우에 적합합니다

- 정보가 많은 사진을 소수의 힘 있는 형태와 관계로 정리하고 싶을 때
- 추상화하더라도 원래 주체가 한눈에 느껴지는 디자인이 필요할 때
- 탁한 회색이나 값싼 캔디 감각 없이 생생하고 편안한 색을 쓰고 싶을 때
- 아트 포스터, 독립 출판, 전시 그래픽, 표지, 벽지, 여러 비율, 폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 103로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 큰 추상 색면 · 거친 콜라주 · 손작업 어긋남 · 원본에서 온 생생한 색 · 편집 여백 · 적응형 글자
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 생생한 색면, 거친 조합, 전문적인 레이아웃, 예술적 여백이 함께 작동하는 추상 편집 포스터 |
| **대표 특징** | 불규칙한 면, 오려 붙인 가장자리, 굵기가 다른 손그림 선, 어긋난 겹침, 웜 화이트 여백, 소량의 글자 |
| **입력 존중 방식** | 정체성, 자세, 관계, 분위기, 원본에서 온 색을 유지하며 핵심 서사를 바꾸지 않음 |
| **활용처** | 독립 출판, 전시 포스터, 아트 표지, 소셜 그래픽, 디자인 전용 이미지, 여러 비율, 벽지 |
<!-- xxd-panel-benefit:end -->

## 변환 논리

```text
가장 기억에 남는 주체와 관계 읽기 → 완전한 장면과 잉여 세부 삭제 → 소수의 큰 색면과 윤곽으로 압축 → 어긋남·겹침·자르기·여백으로 재구성 → 원본의 가장 생명력 있는 색을 밝게 조율 → 소량의 편집 글자로 리듬 완성
```

## 완성작의 특징

- 큰 색면은 사실적 묘사를 피하면서도 윤곽, 자세, 관계로 원본의 정신을 유지합니다.
- 오려 붙인 가장자리, 굵기가 다른 손그림 선, 부분 덮임, 틈, 어긋남이 통제된 즉흥성을 만듭니다.
- 하늘색, 물빛 청록, 연두, 따뜻한 노랑, 코럴, 피치는 원본에서 자연스럽게 나올 때만 쓰고 웜 화이트 여백으로 균형을 잡습니다.
- 시각적 무게중심, 양·음의 형태, 밀도, 여백이 자유로워 보이는 구도를 전문적으로 정리합니다.
- 글자는 손글씨 제목과 현대 편집의 중간에 위치하며 사실적 그림, 완전한 배경, 카툰, 템플릿, 강한 CG 감각을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-103-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 103의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-103 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-103 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-103 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-103 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-103 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-103 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 103 적용, 공통 설정은 한 번만 확정
/xxd-panel-103 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-103`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-103-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-103-prompt.zh-CN.md)
...
| **[xxd-panel-103](https://github.com/nevertoday/xxd-panel-103)** | 선명한 추상 조합 · 불규칙한 큰 색면 · 거친 즉흥성 · 손그림 선 · 전문 레이아웃 · 예술적 여백 |
...
### Xiaoxiaodong 총괄 · 장군 총지휘 Skill · CNY 103

CNY 103을 한 번 결제하면 이 시리즈의 장군 총지휘 Skill(`xxd-panel-all`)을 사용할 수 있습니다. 전체 병사 Skills의 통합 관리, 추천, 지정 파견, 일괄 조정을 지원합니다. WeChat 메시지에 “장군 총지휘 Skill”이라고 적어 주세요.
...
| **병사급** | `xxd-panel-NNN` (현재 001–103) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
