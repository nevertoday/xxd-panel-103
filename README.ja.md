<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 103 プロジェクトバナー" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 103｜鮮彩抽象アセンブリ

### 写真を鮮やかな色面、粗い組み合わせ、上質な余白へ凝縮
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 103 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

写真には、ポスターに必要な量を超える細部が含まれがちです。**Panel 103** は現実の写真を残しながら、記憶に残る輪郭、関係、感情を鮮やかな色面、手描き線、呼吸感のある抽象的な組み合わせへ凝縮します。元の場面の精神的な核心を失わず、より軽やかで明快な画面にします。

### 向いている用途

- 情報量の多い写真を、少数の力強い形と関係へ整理したい場合
- 抽象化しても元の主体を一目で感じられるデザインが必要な場合
- 濁りや安価なキャンディ感を避け、鮮やかで癒やしのある色を使いたい場合
- アートポスター、独立出版、展覧会グラフィック、表紙、壁紙、複数比率、フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 103 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 大きな抽象色面 · 粗いコラージュ · 手作業のずれ · 元写真由来の鮮彩 · 編集的余白 · 適応する文字
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 鮮やかな色面、粗い組み合わせ、プロのレイアウト、芸術的な余白が一体となった抽象編集ポスター |
| **識別しやすい特徴** | 不規則な色面、切り紙の縁、太さの異なる手描き線、ずれた重なり、暖白の余白、少量の文字 |
| **入力の尊重** | 同一性、姿勢、関係、雰囲気、元写真由来の色を保ち、核心の物語を置き換えない |
| **用途** | 独立出版、展覧会ポスター、アート表紙、SNS、デザイン単体、複数比率、壁紙 |
<!-- xxd-panel-benefit:end -->

## 変換ロジック

```text
最も記憶に残る主体と関係を読む → 完全な場面と余分な細部を削る → 少数の大色面と輪郭へ圧縮 → ずれ、重なり、裁ち切り、余白で再構成 → 元写真の最も生命的な色を明るく調整 → 少量の編集文字でリズムを完成
```

## 完成作の特徴

- 大色面は写実描写を避けながら、輪郭、姿勢、関係によって元写真の精神を保ちます。
- 切り紙の縁、太さの違う手描き線、部分的な覆い、隙間、ずれが制御された即興性を生みます。
- 空色、アクア、若草色、暖黄、コーラル、桃色は元写真から自然に導かれる場合のみ使い、暖白の余白で整えます。
- 視覚的重心、正負形、疎密、余白が、無造作に見える構図を専門的に均衡させます。
- 文字は手書き見出しと現代編集の中間。写実、完全な背景、カートゥーン、テンプレート、強いCG感を避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-103-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 103 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-103 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-103 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-103 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-103 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 103 を適用し、共通設定は一度だけ解決
/xxd-panel-103 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-103`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-103-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-103-prompt.zh-CN.md)
...
| **[xxd-panel-103](https://github.com/nevertoday/xxd-panel-103)** | 鮮彩抽象アセンブリ · 不規則な大色面 · 粗い即興性 · 手描き線 · 専門的レイアウト · 芸術的余白 |
...
### Xiaoxiaodong 総控 · 将軍総指揮 Skill · CNY 103

CNY 103 の一回払いで、このシリーズの将軍総指揮 Skill（`xxd-panel-all`）を利用できます。全兵士 Skills の統括、推薦、指名派遣、一括調整に対応します。WeChat では「将軍総指揮 Skill」と記載してください。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–103） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
