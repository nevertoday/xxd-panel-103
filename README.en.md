...
# 🦁 XXD Panel 103｜Vivid Abstract Assembly

### Distil a photograph into vivid colour fields, rough assembly, and artful whitespace
...
The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 103 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

Photographs often carry more detail than a poster needs. **Panel 103** keeps the reality view, then compresses its memorable contours, relationships, and mood into vivid colour fields, hand-drawn lines, and breathable abstract assembly. The result is lighter and clearer without losing the source scene's emotional core.

### Best for

- clarifying a visually busy photograph by reducing it to a few powerful shapes and relationships;
- making an abstract design that remains recognisably grounded in the original subject;
- using lively, restorative colour without falling into muddy grey or cheap candy colour;
- art posters, independent publishing, exhibition graphics, covers, wallpapers, multiple ratios, and directory batches;

### Quick start

> Use XXD Panel 103 on this image and recommend the most suitable composition and size first.
...
|---|---|
| **What you get** | An abstract editorial poster held together by vivid colour fields, rough assembly, professional layout, and artful whitespace |
| **Recognisable signature** | Irregular planes, cut-paper edges, varied hand-drawn lines, misregistered overlaps, warm-white breathing space, and sparse type |
...
```text
identify the most memorable subject and relationships → remove the complete scene and surplus detail → compress into a few large colour fields and contours → rebuild through misregistration, overlap, cropping, and whitespace → brighten the source's most vital colours → finish the rhythm with sparse editorial type
```

## Recognisable finished traits

- Large colour fields avoid realistic rendering while retaining the source's spirit through contour, pose, and relationships.
- Cut-paper edges, varied hand-drawn lines, partial cover, gaps, and misregistration create controlled spontaneity.
- Sky blue, aqua, tender green, warm yellow, coral, or peach appear only when naturally derived from the source, balanced by warm-white space.
- Visual gravity, positive and negative form, density, and whitespace keep the apparently casual composition professionally balanced.
- Sparse type sits between handwritten titling and modern editorial design; avoid realistic illustration, complete backgrounds, cartoons, templates, and overtly computer-generated polish.

All aesthetic constraints live only in the [canonical prompt](references/original-prompt/zh-CN.md). The Skill and runtime adapters resolve delivery variables. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-103-prompt.en.md)
...
```text
/xxd-panel-103 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-103 photo.jpg --prefs last
/xxd-panel-103 photo.jpg --prefs edit
/xxd-panel-103 photo.jpg --prefs new
```
...
/xxd-panel-103 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-103 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-103 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-103 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-103 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-103 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-103 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-103 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-103 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-103 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-103 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 103 to a whole directory; shared settings resolve once
/xxd-panel-103 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-103 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
The current XXD Panel series runs from 001 through 112, and every Panel retains its own independent original prompt and aesthetic logic. The table below is the historical catalog as of this project’s release, listing 001 through 103; the current project is bold.
...
### Xiaoxiaodong Commander · General Command Skill · CNY 103

A one-time CNY 103 purchase unlocks this suite's General Command Skill (`xxd-panel-all`) for roster control, recommendations, Soldier dispatch, and batch coordination. Include “General Command Skill” in your WeChat message.
